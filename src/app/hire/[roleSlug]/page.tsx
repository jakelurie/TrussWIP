...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${role.hiringTitle} | Truss`,
    description: `Find and book verified ${role.title}s across 15 major US cities. See rates, reviews, and availability. Book directly on Truss.`,
    openGraph: {
      title: role.hiringTitle,
      description: `Browse verified ${role.title}s in major AV markets. Transparent rates, direct booking.`,
      url: `https://trusswork.org/hire/${role.slug}`,
    },
  };
}
    ...
    <main className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
      ...
      <section className="text-center py-12 border-t border-ink/[0.08]">
        <h2 className="font-heading text-xl font-bold tracking-tight mb-3">
          Ready to book a{/^[aeiou]/i.test(role.title) ? "n" : ""} {role.title}?
        </h2>
